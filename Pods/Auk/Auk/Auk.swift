  ...
  */
  ...
  init(scrollView: UIScrollView) {
    ...
    DispatchQueue.main.async {
        self.scrollViewDelegate.delegate = self.scrollView?.delegate
  ...
  func createPageIndicator() {
    ...
    DispatchQueue.main.async {
        guard let scrollView = self.scrollView, let superview = scrollView.superview else { return }
        
        let container = AukPageIndicatorContainer()
        container.didTapPageControlCallback = self.didTapPageControl
        superview.insertSubview(container, aboveSubview: scrollView)
        self.pageIndicatorContainer = container
        container.setup(self.settings, scrollView: scrollView)
